...
import _app from '@/modules/_appjs/bridge'
import axios from 'axios'
import ua from '../_ua/browserUa'
...
function wx_share(title,desc,link,imgUrl){
  ...
  // return;
  axios.get(siteInfo.host + '/user-center/wxApi/share?url='+encodeURIComponent(window.location.href)
  // axios.post('/api/index.php?act=wx&op=ajaxGetSignPackage',
  //   qs.stringify({request_url:})
    // qs.stringify({request_url:encodeURIComponent(window.location.href.split('#')[0])})
  ).then(res => {
      share_config(title,desc,link,imgUrl,res.data.content);
  }).catch(err => {
