<template>
  <div class="fpm-agreement">
    ...
    <h1 class="agreement-title">“一起善源”平台扶贫电子码采购协议</h1>
    ...
    <div class="agreement-con">
      ...
      <p>采购金额总计：&ensp;￥<span>{{orderInfo.goodsNum * orderInfo.goodsPrice}}&ensp;元</span></p>
    ...
    <div>
      ...
      <p class="agreement-con">开票要求：{{showRequest}}</p>
    ...
    <div>
      ...
      <p class="agreement-con mar-10">签署日期： {{createdAt}}</p>
    </div>
    <p class="agreement-bottom iphonex-margin-bottom">&copy;17shanyuan.com</p>
    <div class="iphonex-bottom-blank"></div>
    <div class="xy-bottom-btn-con">
      <button class="xy-bottom-btn" @click="postOrderData()" :disabled="isDisable" >同意并下单</button>
      <div class="iphonex-bottom-blank"></div>
    </div>
  </div>
</template>
<script>
  ...
  import {getShowFpmOrderInfo, postFpmOrderInfoAgree} from "@/requests/fpmOrder.js"
  export default{
    data(){
      return {
        fTitle:'扶贫码采购',
    ...
    mounted() {
      this.$nextTick(()=>{
        ...
        this.$header.middleFn({
          type:'img',
          url:require('../../../../static/images/common/step_3.png'),
          callback:function () {
          }
        })
      })
    ...
    methods: {
      getOnlineData(){
        this.biz_id = ls.getLsItem("postOrderFirstId");
//        console.log(this.biz_id)
        if(this.biz_id){
          getShowFpmOrderInfo(this.biz_id).then((res)=>{
            if(res && res.data.code == 200){
              let getObj = res.data.data.BizOrderSaveBo;
              this.orderInfo.creatorType = 10;
              this.orderInfo.orderSource = this.platform == ''? this.platform='app' : this.platform;
              this.orderInfo.purchaserName = getObj.purchaserName;
              this.orderInfo.identifyNo = getObj.identifyNo;
              this.orderInfo.companyAddress = getObj.companyAddress;
              this.orderInfo.companyTel = getObj.companyTel;
              this.orderInfo.bankName = getObj.bankName;
              this.orderInfo.bankAccount = getObj.bankAccount;
              this.orderInfo.invoiceType = getObj.invoiceType;
              let invoice = getObj.invoiceType == 0 ? '增值税普通发票' : '增值税专用发票';
              this.orderInfo.showInvoiceType = invoice;
              this.orderInfo.isSpecial = getObj.isSpecial;
              this.orderInfo.specialInfo = getObj.specialInfo;
              let requestStr = getObj.isSpecial == 0 ?'无特殊要求':getObj.specialInfo;
              this.showRequest = requestStr;
              this.orderInfo.invoiceReciverName = getObj.invoiceReciverName;
              this.orderInfo.invoiceReciverTel = getObj.invoiceReciverTel;
              this.orderInfo.invoiceContactAddress = getObj.invoiceContactAddress;
              this.orderInfo.invoiceExactAddress = getObj.invoiceExactAddress;
              this.orderInfo.pointsUnoinId = getObj.pointsUnoinId;
              this.orderInfo.pointsUnoinName = getObj.pointsUnoinName;
              this.orderInfo.email = getObj.email;
              this.orderInfo.signatusre = getObj.signatusre;
              this.orderInfo.goodsPrice = parseInt(getObj.goodsPrice)/100;
              this.orderInfo.goodsNum = getObj.goodsNum;
              let _year = getObj.createdAt.slice(0,4);
              let _month = getObj.createdAt.slice(5,7);
              let _day = getObj.createdAt.slice(8,10);
              this.createdAt = _year+'  年 ' +_month+'  月 '+_day+'  日 ';

            }
          })
        }else{
//          console.log('postOrderFirstId 丢失');
        }
      },
      postOrderData(){
        let that = this;
        ...
        if(that.isDisable){
          /*console.log(that.isDisable)*/
          postFpmOrderInfoAgree(this.orderInfo).then((res) => {
            if (res && res.data.code == 200) {
              that.order_id = res.data.data.mainId;
              that.result = 'true';
              this.removeLsItem('postOrderFirstId');
              this.appOpen('/fpmOrderResults', {result: this.result, orderId: that.order_id})
            } else {
              that.result = 'false';
              this.removeLsItem('postOrderFirstId');
              this.appOpen('/fpmOrderResults', {result: this.result, orderId: that.order_id})
