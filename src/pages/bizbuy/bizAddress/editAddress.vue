      ...
      <div v-show="true">
        ...
        <div class="white">
          <div class="normal big">
            <textarea class="addr-detail-color" v-model="userAddress.exactAddress" placeholder="请填写详细地址，不少于5个字"></textarea>
          </div>
        </div>
...
//                this.$router.replace({path: '/fpmPurchasingInfo'});
//                this.appOpen('/fpmPurchasingInfo',null,false,false,true);
              }else{
                this.$router.replace({path: '/bizAddrCd'});
//                this.appOpen('/bizAddrCd',null,false,false,true);
                /*console.log('looktgRefreshFn')
                try{
                  this.appNotify({callback:'looktgRefreshFn'})
                }catch (err){
                  console.log(err)
                }
                setTimeout(()=>{
          ...
          type.editAddressInfo({
            ...
            if (res.data.code == 200) {
              ...
              this.$router.replace({path: '/bizAddrCd'});
//              this.appOpen('/bizAddrCd',null,false,false,true);
              /*console.log('looktgRefreshFn success')
              try{
                this.appNotify({callback:'looktgRefreshFn'})
              }catch (err){
                console.log(err)
